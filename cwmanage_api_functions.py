...
def add_configuration_to_ticket(ticket):
    for ticket in ticket:
        ...
        if ticket_configurations == []:
            ...
            regex_parse = {}
            try:
                regex_parse['domotz_id'] = re.search(regex_domotz_device_id,ticket_note).group(1)
            except:
            ...
            for i in regex_parse:
                if regex_parse[i] != None:
                    try:
                        if i == "domotz_id":
                            configuration_id = requests.get(headers=constants.headers_cw, url=f"{constants.cw_manage_url}/company/configurations/", params={"customFieldConditions":f"caption='Domotz ID' AND value={regex_parse[i]}","fields":"id"}).json()
                        else:
                            configuration_id = requests.get(headers=constants.headers_cw, url=f"{constants.cw_manage_url}/company/configurations/", params={"conditions":f"{i}='{regex_parse[i]}' AND company/name='{ticket_company}'","fields":"id"}).json()
                        if configuration_id != []:
